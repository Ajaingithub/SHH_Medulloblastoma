library(Seurat)
savedir = "/diazlab/data3/.abhinav/projects/SHH/snRNA/removed_samples_BO/RL_like_clus_11/"
obj_RL <- readRDS(paste(savedir,"saveRDS_obj/obj_RL.RDS",sep = ""))

### Removing DOD4182 very low cell number i.e. 13
cells <- rownames(obj_RL@meta.data[grep("UCSFDOD4182",obj_RL@meta.data$orig.ident,invert=TRUE),])
obj_RL_subset <- subset(obj_RL, cells = cells)

DefaultAssay(obj_RL_subset) <- "RNA"
obj_RL_subset <- NormalizeData(obj_RL_subset, normalization.method = "LogNormalize", scale.factor = 10000)
obj_RL_subset <- ScaleData(obj_RL_subset, features = rownames(obj_RL_subset))
obj_RL_subset <- FindVariableFeatures(obj_RL_subset, selection.method = "vst", nfeatures = 2000)
obj_RL_subset <- RunPCA(obj_RL_subset, features = VariableFeatures(object = obj_RL_subset))
obj_RL_subset <- FindNeighbors(obj_RL_subset, dims = 1:30)
obj_RL_subset <- RunUMAP(obj_RL_subset, dims = 1:30)
obj_RL_subset <- FindClusters(obj_RL_subset, resolution = 0.4)

pdf(paste(savedir,"UMAP/RL_UMAP_individual.pdf",sep=""))
DimPlot(obj_RL_subset, reduction = "umap", group.by = "orig.ident", label = TRUE) + NoLegend()
dev.off()

DefaultAssay(obj_RL_subset) <- "MAGIC_RNA"
genes <- c("OTX2","OTX2-AS1","UNC5D","LINGO2","TRPM3","HOXA1","HOXA3","HOXA10","HOXA11-AS","HOXA5","HOXA6")

rm(plot_list)
plot_list <- list()
for(i in 1:length(genes)){
  plot_list[[i]] <- FeaturePlot(obj_RL_subset, features = genes[i], reduction = "umap") + scale_color_gradientn(colors = ArchRPalettes$solarExtra)
}

dir.create(paste(savedir,"featureplot",sep =""), showWarnings = FALSE)
pdf(paste(savedir,"featureplot/RL_HOX_genes_non_integrated.pdf",sep = ""))
plot_list
dev.off()

s_genes <- cc.genes$s.genes
g2m_genes <- cc.genes$g2m.genes
obj_RL_subset <- CellCycleScoring(obj_RL_subset, g2m.features = g2m_genes, s.features = s_genes)

source("/diazlab/data3/.abhinav/resources/all_scripts/R/scCITESeq_sctransform_V2.R")
objname = "obj"
Assay = "RNA"
process = "sctransform"
CD4_RNA_sctransformed <- sctransform_V2_integration(obj = obj_RL_subset, saveDir = savedir, ngenes = 4000,
...
for (i in 1:length(res)) {
  ...
  CD4_integrated_RNA_NN_cluster <- cluster_UMAP_and_QC(obj_path = CD4_integrated_RNA, dims = 30, res = res[i], saveDir=savedir, Assay = Assay,
                                                       QC_features = c("nCount_RNA","nFeature_RNA","S.Score","G2M.Score"), objname = "obj_RL_subset",
                                                       process = process, col_sel = c("orig.ident","tumor_type","sex","age"))
}

DefaultAssay(CD4_integrated_RNA_NN_cluster) <- "MAGIC_RNA"
genes <- c("OTX2","OTX2-AS1","UNC5D","LINGO2","HOXA1","HOXA3","HOXA10","HOXA11-AS","HOXA5","HOXA6")

rm(plot_list)
plot_list <- list()
for(i in 1:length(genes)){
  plot_list[[i]] <- FeaturePlot(CD4_integrated_RNA_NN_cluster, features = genes[i], reduction = "umap") + scale_color_gradientn(colors = ArchRPalettes$solarExtra)
}

dir.create(paste(savedir,"featureplot",sep =""), showWarnings = FALSE)
pdf(paste(savedir,"featureplot/RL_HOX_genes.pdf",sep = ""))
plot_list
dev.off()
